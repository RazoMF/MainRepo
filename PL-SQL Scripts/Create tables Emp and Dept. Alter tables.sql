/*Script: Создадим табицы сотрудников и депаратментов*/
CREATE TABLE EMP
...
/*Добавим ограничения на таблицы*/
alter table emp add constraint emp_pk primary key(empno);
alter table dept add constraint dept_pk primary key(deptno);
alter table emp add constraint emp_fk_dept foreign key(deptno) references dept;
alter table emp add constraint emp_fk_emp foreign key(mgr) references emp; --добавим ключ 
/*Можно создать индекс, но не имеет смысла для первичного ключа empno 
create index emp_empno_idx on emp(empno); */


/*
Теперь из таблиц не получится удалить данные, нсли они завязаны на другую таблицу.
Прежде чем делать truncate, придётся удалить ограничения 
